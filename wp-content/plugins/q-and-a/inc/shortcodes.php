...
function qahome_shortcode( $atts ) {
	...
	if ( $id ) { // Show a single entry //
		...
		while( $qa_faqs->have_posts() ): $qa_faqs->the_post();
			...
			';
			$qaplus_shortcode_output .= '<h5 class="qa-faq-title"><a class="qa-faq-anchor" href="' . get_permalink() . '">'. get_the_title().'</a></h5>
			';
		...
		$qaplus_shortcode_output .=  '<div class="qa-category">
			<h3 class="faq-catname">' . $category->name . '</h3>
			';	
		...
		if ( $categories ) {
			...
			foreach ( $categories as $category ) {
				...
				$qaplus_shortcode_output .=  '<div class="qa-category">
				<h3 class="faq-catname">' . $catheader . '</h3>
				';
